...
class TritechMicron(object):
    ...
    def open(self):
        ...
        self.node.get_logger().info("##########rebooted")  
        ...
        self.set(force=True)        
        self.node.get_logger().info("#####################force = true")
    ...
    def get(self, message=None, wait=20):
        ...
        while message is None or datetime.datetime.now() < end:
            try:                   
                ...
                print(f"Received Reply ID: {reply.id}, Expected: {message}")                   
                # Update state if mtAlive.
                if reply.id == Message.ALIVE:
                    print(f"msg alive == reply id")
                ...
                # Keep trying.
                self.node.get_logger().warning(f"Packet corrupted")
                continue
    ...
    def _ping(self):
        ...
        current_time_datetime = datetime.timedelta(
            hours=now.hour,
            minutes=now.minute,
            seconds=now.second,
            microseconds=0)
        ...
        current_time = (now.hour * 3600 * 1000 + now.minute * 60 * 1000 + now.second * 1000 + now.microsecond)
        print("calculating payload")
        # payload = bitstring.pack("uintle:32", current_time)
        # payload = bitstring.pack("uintle:8 , uintle:8, uintle:8, uintle:8, uintle:8, uintle:8, uintle:32",
        #                          255, 20, 7, 25, 128, 20,current_time)
        # Reset offset for up time.
        self._time_offset = current_time_datetime - self.up_time
        print(f"time offset: {self._time_offset.total_seconds()}, current time: {current_time}")
        payload = bitstring.pack("uintle:32", current_time)

        print(f"payload.length: {payload.length}")

        # Send command.
        # print("sendind SEND_DATA")
        # self.send(Message.SEND_DATA, payload)

        # print("sendind SEND_BB_USER")
        # self.send(Message.SEND_BB_USER)
        # self.get(Message.BB_USER_DATA)

        # print("sendind SEND_VERSION")
    ...
    def scan(self, callback):
        ...
        while not self.preempted:
            ...
            # Get the scan data.
            try:               
                print("Message.Head_Data")
                data = self.get(Message.HEAD_DATA, wait=5).payload
                timeout_count = 0
                ...
                # Try again.
                continue
            print("++++++++++++trying to parse head data")
            try:
                print("++++++++++++yes")
    ...
    def __update_state(self, alive):
        ...
        self.node.get_logger().info(f"RECENTERING: {self.recentering}")
        self.node.get_logger().info(f"CENTRED:     {self.centred}")
        self.node.get_logger().info(f"MOTORING:    {self.motoring}")
        self.node.get_logger().info(f"MOTOR ON:    {self.motor_on}")
        self.node.get_logger().info(f"CLOCKWISE:   {self.scanright}")
        self.node.get_logger().info(f"SCANNING:    {self.scanning}")
        self.node.get_logger().info(f"NO PARAMS:   {self.no_params}")
        self.node.get_logger().info(f"HAS CFG:     {self.has_cfg}")
        print(f"=============================") 
