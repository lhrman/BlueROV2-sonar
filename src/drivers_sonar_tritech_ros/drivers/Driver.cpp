#include "drivers_sonar_tritech/Driver.hpp"
#include <errno.h>
#include <fcntl.h>
#include <netdb.h>
...
#include <time.h>
#include <unistd.h>
// #include <base-logging/Logging.hpp> // TODO: log youself
...
#include <sstream>
#include <stdexcept>
#include "drivers_sonar_tritech/IOListener.hpp"
#include "drivers_sonar_tritech/IOStream.hpp"
#include "drivers_sonar_tritech/TestStream.hpp"
#include "drivers_sonar_tritech/Timeout.hpp"
#include "drivers_sonar_tritech/URI.hpp"
...
Driver::Driver(int max_packet_size, bool extract_last, bool debug)
...
IOStream* Driver::getMainStream() const { return m_stream; }
...
static void validateURIScheme(std::string const& scheme) {
   char const* knownSchemes[7] = {"serial", "tcp",  "udp", "udpserver",
                                  "file",   "test", "fd"};
   for (int i = 0; i < 7; ++i) {
...
/** Backward-compatibility code to handle the old syntax
 * udp://host:remote_port:local_port
 *
...
static URI backwardParseBidirectionalUDP(string const& uri_string) {
   ...
   string new_uri =
       uri_string.substr(0, last_colon) + "?local_port=" + local_port;
   ...
   if (scheme == "serial") {  // serial://DEVICE:baudrate
      if (uri.getPort() == 0) {
         throw std::invalid_argument(
             "missing baud rate specification in serial URI");
      }
      openSerial(uri.getHost(), uri.getPort(),
                 SerialConfiguration::fromURI(uri));
      ...
      if (!dynamic_cast<TestStream*>(getMainStream())) openTestMode();
...
void Driver::openURI_UDP(URI const& uri) {
   ...
   if (local_port.empty() && ignore_connrefused.empty()) {
      std::cerr << "Warning: udp://host:port streams historically would report "
                   "connection "
                   "refused errors. This default behavior will change in the "
                   "future."
                << std::endl;
      std::cerr << "Warning: Set the ignore_connrefused option to 1 to update "
                   "to the new "
   ...
   if (!local_port.empty() && connected.empty()) {
      std::cerr
          << "Warning: udp://host:remote_port?local_port=PORT historically was "
             "not "
             "connecting the socket, which means that any remote host could "
             "send messages to the local socket."
          << std::endl;
      std::cerr
          << "Warning: This default behavior will change in the future. Set "
             "the "
             "connected option to 1 to update to the new behavior, that is "
             "allowing only the specified remote host to send packets."
          << std::endl;
      std::cerr
          << "Warning: Set to 0 to keep the current behavior even after the "
             "default is changed "
          << std::endl;
   }
   ...
   if (ignore_connrefused.empty()) {
      ignore_connrefused =
          is_connected ? (local_port.empty() ? "0" : "1") : "1";
   }
...
void Driver::openTestMode() { setMainStream(new TestStream); }
...
struct AddrinfoGuard {
   addrinfo* ptr;
   AddrinfoGuard(addrinfo* ptr) : ptr(ptr) {}
...
static int createIPServerSocket(const char* port, addrinfo const& hints) {
   ...
   if (ret != 0) throw UnixError("cannot resolve server port " + string(port));
...
static int createIPClientSocket(const char* hostname, const char* port,
...
static pair<sockaddr_storage, size_t> connectIPSocket(int fd,
                                                      const char* hostname,
                                                      const char* port,
                                                      addrinfo const& hints) {
   addrinfo* candidates;
...
void Driver::openIPClient(std::string const& hostname, int port,
                          addrinfo const& hints) {
   int sfd = createIPClientSocket(hostname.c_str(),
                                  lexical_cast<string>(port).c_str(), hints,
                                  nullptr, nullptr);
...
void Driver::openUDP(std::string const& hostname, int port) {
   if (hostname.empty()) {
      std::cerr << "Warning: openUDP: providing an empty hostname is "
                   "deprecated, use openUDPServer instead"
                << std::endl;
...
void Driver::openUDPBidirectional(std::string const& hostname, int remote_port,
                                  int local_port, bool ignore_connrefused,
                                  bool connected, bool ignore_hostunreach,
   ...
   int sfd = createIPServerSocket(lexical_cast<string>(local_port).c_str(),
                                  local_hints);
   ...
   if (connected) {
      peer = connectIPSocket(sfd, hostname.c_str(),
                             lexical_cast<string>(remote_port).c_str(),
                             remote_hints);
   } else {
      int fd = createIPClientSocket(hostname.c_str(),
                                    lexical_cast<string>(remote_port).c_str(),
                                    remote_hints, &peer.first, &peer.second);
      ::close(fd);
   }
   auto stream = new UDPServerStream(
       sfd, true, reinterpret_cast<sockaddr*>(&peer.first), &peer.second);
...
SerialConfiguration Driver::parseSerialConfiguration(
    std::string const& description) {
   boost::regex ex =
       boost::regex("^([5-8])([neo])([12])$", boost::regex_constants::icase);
   ...
   serial_config.parity =
       static_cast<ParityChecking>(toupper(sm[2].str().at(0)));
...
void Driver::setSerialConfiguration(SerialConfiguration const& serial_config) {
   ...
   if (tcgetattr(fd, &tio)) {
      throw UnixError(
          "Driver::setSerialConfiguration: Failed to get terminal info\n");
   }
   ...
   if (tcsetattr(fd, TCSANOW, &tio) != 0) {
      throw UnixError(
          "Driver::setSerialConfiguration: Failed to set terminal info\n");
   }
}
...
#else
         std::cerr << "Non-standard baud rate selected. This is only supported "
                      "on linux."
                   << std::endl;
         return false;
#endif
   ...
   if (custom_rate) {
      ...
      if (ss.custom_divisor == 0) {
         std::cerr << "Cannot set custom serial rate to " << brate
                   << " as the calculated divisor is zero for baud_base of "
                   << ss.baud_base << "." << std::endl;
         return false;
      }
...
void Driver::pullBytesFromInternal(uint8_t* buffer, int skip, int size) {
   ...
   memmove(internal_buffer, internal_buffer + total_size, new_internal_size);
...
int Driver::readRaw(uint8_t* buffer, int out_buffer_size,
   ...
   auto inter_byte_timeout =
       inter_byte_timeout_.isNull() ? packet_timeout : inter_byte_timeout_;
   ...
   while (buffer_fill < out_buffer_size && now <= global_deadline) {
      ...
      int c =
          m_stream->read(buffer + buffer_fill, out_buffer_size - buffer_fill);
      now = Time::now();
...
pair<int, bool> Driver::readPacketInternal(uint8_t* buffer,
   ...
   if (out_buffer_size < MAX_PACKET_SIZE)
      throw length_error("readPacket(): provided buffer too small (got " +
                         lexical_cast<string>(out_buffer_size) +
                         ", expected at least " +
   ...
   while (true) {
      ...
      if (c > 0) {
         ...
         // cerr << "received: " << printable_com(buffer + buffer_size, c) <<
         // endl;
      ...
      if (internal_buffer_size == (size_t)MAX_PACKET_SIZE)
         throw length_error(
             "readPacket(): current packet too large for buffer");
   }
...
void Driver::setReadTimeout(Time const& timeout) { m_read_timeout = timeout; }
Time Driver::getReadTimeout() const { return m_read_timeout; }
int Driver::readPacket(uint8_t* buffer, int buffer_size) {
   return readPacket(buffer, buffer_size, getReadTimeout(), getReadTimeout());
}
int Driver::readPacket(uint8_t* buffer, int buffer_size,
                       Time const& packet_timeout) {
   return readPacket(buffer, buffer_size, packet_timeout, packet_timeout);
}
   ...
   if (!isValid()) {
      ...
      if (result.first) {
         ...
         throw TimeoutError(TimeoutError::PACKET,
                            "readPacket(): no packet in the internal "
   ...
   while (true) {
      ...
      if (packet_timeout.isNull()) {
         throw TimeoutError(TimeoutError::FIRST_BYTE,
                            "readPacket(): no data to read while a "
                            "packet_timeout of 0 was given");
      }
      ...
      if (now > deadline) {
         throw TimeoutError(
             ...
             "readPacket(): no data after waiting " +
                 lexical_cast<string>((now - start_time).toMilliseconds()) +
                 "ms");
      }
...
void Driver::setWriteTimeout(Time const& timeout) { m_write_timeout = timeout; }
Time Driver::getWriteTimeout() const { return m_write_timeout; }
...
bool Driver::writePacket(uint8_t const* buffer, int buffer_size, int timeout) {
   ...
   while (true) {
      ...
      if (!m_stream->waitWrite(
              Time::fromMicroseconds(remaining_timeout * 1000))) {
...
bool Driver::eof() const {
   if (!m_stream) throw std::runtime_error("eof(): invalid stream");
   return m_stream->eof();
}
