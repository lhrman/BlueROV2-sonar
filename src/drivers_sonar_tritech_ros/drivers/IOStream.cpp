...
// #include <base-logging/Logging.hpp> //TODO: log yourself
...

{
   if (setNonBlockingFlag(fd)) {
      std::cerr << "Warning: FD given to Driver::setFileDescriptor is set as "
                   "blocking, "
                   "setting the NONBLOCK flag"
                << std::endl;
   }
}
FDStream::~FDStream() {
...
void FDStream::setAutoClose(bool flag) { m_auto_close = flag; }
...
bool FDStream::waitRead(base::Time const& timeout) {
   ...
   timeval timeout_spec = {static_cast<time_t>(timeout.toSeconds()),
                           suseconds_t(timeout.toMicroseconds() % 1000000)};
...
bool FDStream::eof() const { return m_eof; }
...
UDPServerStream::UDPServerStream(int fd, bool auto_close,
                                 struct sockaddr* si_other, size_t* s_len)
    : FDStream(fd, auto_close),
      m_si_other(*si_other),
      m_s_len(*s_len),
...
bool UDPServerStream::waitRead(base::Time const& timeout) {
   ...
   while (now <= deadline) {
      if (!FDStream::waitRead(deadline - now)) return false;
...
pair<ssize_t, int> UDPServerStream::recvfrom(uint8_t* buffer,
...
pair<ssize_t, int> UDPServerStream::sendto(uint8_t const* buffer,
                                           size_t buffer_size) {
...
size_t UDPServerStream::write(uint8_t const* buffer, size_t buffer_size) {
   if (!m_has_other) return buffer_size;
