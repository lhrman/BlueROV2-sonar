...
int SeaNetPacket::isValidPacket(uint8_t const *buffer, size_t buffer_size,
   ...
   if (readPos > 0) {
      std::cerr << "Warning: Corrupted packet: Skipping " << readPos
                << " bytes because no start was found." << std::endl;
      return -readPos;
   }
   ...
   if (len != hexlen) {
      std::cerr << "Warning: Corrupted packet: Binary packet size differs from "
                   "hexadecimal: bin:"
   ...
   if (buffer[len - 1] == PACKET_END) {
      if (debug) {
         std::cout << "DEBUG: Found packet of type " << type << " and size "
                   << len << std::endl;
      }
         ...
         case mtHeadData:
            if (len > 45) {
               ...
               if ((DeviceType)buffer[7] == PROFILINGSONAR) {
                  uint16_t data_count =
                      buffer[38] | (((uint16_t)buffer[39]) << 8);
                  uint16_t data_bytes = data_count * 2;
                  if ((uint16_t)len == data_bytes + 51 &&
                      (uint16_t)len == total + 14)
                     return len;
               } else {
                  uint16_t data_bytes =
                      buffer[42] | (((uint16_t)buffer[43]) << 8);
                  if ((uint16_t)len == data_bytes + 45 &&
                      (uint16_t)len == total + 14)
                     return len;
               }
               std::cerr << "Warning: Corrupted mtHeadData: Size miss match"
                         << std::endl;
            }
            break;
         case mtAuxData:
            if (len > 16) {
               ...
               std::cerr << "Warning: Corrupted mtAuxData: Size miss match"
                         << std::endl;
            }
            break;
         case mtVersionData:
      ...
      std::cerr
          << "Warning: Corrupted packet: Wrong packet size for packet type:"
          << type << " . size:" << len;
      return -1;
   }
   std::cerr << "Warning: Corrupted packet (possibly type " << type
             << "): no message end was found, packet size " << len << std::endl;
   return -1;
}
...
std::vector<uint8_t> SeaNetPacket::createPaket(DeviceType device_type,
                                               ...
                                               bool debug) {
   if (debug) {
      std::cout << "DEBUG: Create new packet of type " << packet_type
                << " with payload_size " << payload_size << std::endl;
   }
   ...
   size_t size = payload_size +
                 14;  // 14 Bytes is the size of a packet without user payload
...
uint8_t *SeaNetPacket::getPacketPtr() { return &packet[0]; }

void SeaNetPacket::setSize(size_t size) { this->size = size; }

void SeaNetPacket::invalidate() { valid = false; }
...
PacketType SeaNetPacket::getPacketType() const {
   if (!isValid()) throw std::runtime_error("Package is not valid!");
...
void SeaNetPacket::decodeAliveData(AliveData &data) const {
   if (getPacketType() != mtAlive)
      throw std::runtime_error(
          "Cannot decode AliveData. Wrong packet type is buffered.");
   ...
   std::cout << "aliveData - ready: " << data.ready << std::endl;
   std::cout << "aliveData - motor_on: " << data.motor_on << std::endl;
   std::cout << "aliveData - scanning: " << data.scanning << std::endl;
   std::cout << "aliveData - no_config: " << data.no_config << std::endl;
   std::cout << "aliveData - config_send: " << data.config_send << std::endl;
}

void SeaNetPacket::decodeHeadData(ImagingHeadData &data) const {
   if (getPacketType() != mtHeadData)
      throw std::runtime_error(
          "Cannot decode mtHeadData. Wrong packet type is buffered.");

   if (getSenderType() != IMAGINGSONAR)
      throw std::runtime_error(
          "Cannot decode mtHeadData. Sender isn't an imaging sonar.");
   ...
   data.txn =
       packet[22] | (packet[23] << 8) | (packet[24] << 16) | (packet[25] << 24);
   data.gain = packet[26];
...
void SeaNetPacket::decodeHeadData(ProfilingHeadData &data) const {
   ...
   if (getSenderType() != PROFILINGSONAR)
      throw std::runtime_error(
          "Cannot decode mtHeadData. Sender isn't a profiling sonar.");
...
void SeaNetPacket::decodeVersionData(VersionData &version) const {
   ...
   version.programmLength =
       packet[17] | (packet[18] << 8) | (packet[19] << 16) | (packet[20] << 24);
