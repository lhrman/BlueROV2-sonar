#include "drivers_sonar_tritech/Time.hpp"
#include <math.h>
#include <stdio.h>
#include <sys/time.h>
#include <time.h>
...
timeval Time::toTimeval() const {
   timeval tv = {static_cast<time_t>(microseconds / UsecPerSec),
                 static_cast<suseconds_t>(microseconds % UsecPerSec)};
   return tv;
}
...
Time Time::max() { return Time(numeric_limits<int64_t>::max()); }

Time Time::fromTimeValues(int year, int month, int day, int hour, int minute,
...
Time Time::fromString(const string& stringTime, Time::Resolution resolution,
   ...
   if (resolution > Seconds) {
      ...
      if (!match) {
         throw runtime_error(
             "Time::fromString: required resolution format does not match the "
             "given string '" +
   ...
   if (NULL == strptime(mainTime.c_str(), mainFormat.c_str(), &tm)) {
      throw runtime_error("Time::fromString failed: " + mainTime +
   ...
   if (tzInfo.empty()) {
      ...
      // The problem we try to work around here is that mktime converts a
      // broken-down time in the local time zone to UTC, but what we have is a
      // time in UTC and want to convert it to UTC

      // 'tm' is the time represented by the string. It is `tzInfo` seconds away
      // from UTC
      int64_t tzOffset = Time::tzInfoToSeconds(tzInfo);
      tm.tm_sec += tzOffset;

      // OK, now `tm` is the string's time in UTC. Note that mktime handles
      // wraparounds
      tm.tm_isdst = -1;
      time = mktime(&tm);

      // Since mktime expected `tm` to be in local time, `time` is UTC with the
      // local time zone offset as it was at `tm` applied in opposite (e.g. time
      // would be UTC-3 if executed on a machine whose current time zone is
      // UTC+3). We have to correct with the local offset to finally get UTC
      int64_t localTimezoneOffset = Time::getTimezoneOffset(time);
      time -= localTimezoneOffset;
   }

   return Time(static_cast<int64_t>(time) * UsecPerSec +
               static_cast<int64_t>(usecs));
}
...
int64_t Time::tzInfoToSeconds(const string& tzInfo) {
   ...
   if (r != 2 || tzInfo.size() != 5) {
      throw invalid_argument(
          "base::Time::tzInfoToSeconds: parsing of "
          "timezone offset '" +
          tzInfo + "' failed");
   }
   tzOffset = hours * 3600;
...
ostream& base::operator<<(ostream& io, const Time& time) {
   ...
   io << (microsecs / 1000000) << setfill('0') << "." << setw(3)
      << (llabs(microsecs) / 1000) % 1000 << "." << setw(3)
      << (llabs(microsecs) % 1000) << setfill(' ');
