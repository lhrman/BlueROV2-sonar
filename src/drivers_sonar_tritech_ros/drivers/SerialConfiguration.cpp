#include "drivers_sonar_tritech/SerialConfiguration.hpp"
#include <cstring>
#include <stdexcept>
...
SerialConfiguration SerialConfiguration::fromURI(URI const& uri) {
   ...
   if (!byte_size_s.empty()) {
      ...
      if (byte_size < 5 || byte_size > 8) {
         throw std::invalid_argument(
             "invalid byte_size parameter " + byte_size_s +
             "in URI, "
   ...
   if (parity == "none") {
      ...
      throw std::invalid_argument("invalid parity parameter " + parity +
                                  "in URI, expected one of "
                                  "none, even or odd");
   }
   ...
   if (stop == "1") {
      ...
      throw std::invalid_argument("invalid stop parameter " + stop +
                                  " in URI, expected 1 or 2");
   }
