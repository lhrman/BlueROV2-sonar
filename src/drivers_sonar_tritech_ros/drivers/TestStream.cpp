...
#include <iomanip>
#include <sstream>
#include "drivers_sonar_tritech/Exceptions.hpp"

using namespace std;
using namespace iodrivers_base;

TestStream::TestStream() : m_mock_mode(false), m_eof(false) {}
...
bool TestStream::waitWrite(base::Time const& timeout) { return true; }

void TestStream::EXPECT_REPLY(std::vector<uint8_t> const& expectation,
                              std::vector<uint8_t> const& reply) {
   if (!m_mock_mode) throw MockContextException();
...
size_t TestStream::write(uint8_t const* buffer, size_t buffer_size) {
   if (m_mock_mode) {
      ...
      if (expectations.empty()) {
         ...
         for (std::vector<uint8_t>::const_iterator it = from_driver.begin();
              it != from_driver.end(); ++it)
            msg << " " << setfill('0') << setw(2) << hex
                << static_cast<int>(*it);
         throw std::runtime_error(msg.str());
      }

      if (from_driver == expectations.front()) {
         to_driver.insert(to_driver.end(), replies.front().begin(),
                          replies.front().end());
         from_driver.clear();
         ...
         msg << "\nExpected";
         for (std::vector<uint8_t>::const_iterator it = expected.begin();
              it != expected.end(); ++it)
            msg << " " << setfill('0') << setw(2) << hex
                << static_cast<int>(*it);
         msg << "\nBut got ";
...
void TestStream::clear() { to_driver.clear(); }
...
bool TestStream::expectationsAreEmpty() { return (expectations.empty()); }
...
bool TestStream::eof() const { return m_eof; }

void TestStream::setEOF(bool flag) { m_eof = flag; }
