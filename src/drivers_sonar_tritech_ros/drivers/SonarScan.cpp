#include "drivers_sonar_tritech/SonarScan.hpp"
#include <memory.h>
#include <stdint.h>
#include <iostream>
#include <sstream>
...
SonarScan::SonarScan(uint16_t number_of_beams, uint16_t number_of_bins,
                     Angle start_bearing, Angle angular_resolution,
                     bool memory_layout_column) {
   init(number_of_beams, number_of_bins, start_bearing, angular_resolution,
        memory_layout_column);
}

SonarScan::SonarScan(const SonarScan& other, bool bcopy) { init(other, bcopy); }
...
void SonarScan::init(const SonarScan& other, bool bcopy) {
   init(other.number_of_beams, other.number_of_bins, other.start_bearing,
        other.angular_resolution, other.memory_layout_column);
   time = other.time;
...
void SonarScan::init(uint16_t number_of_beams, uint16_t number_of_bins,
                     Angle start_bearing, Angle angular_resolution,
                     bool memory_layout_column, int val) {
   // change size if the sonar scan does not fit
   if (this->number_of_beams != number_of_beams ||
...
bool SonarScan::hasSonarBeam(const Angle bearing) const {
   ...
   if (time_beams[index].microseconds != 0) return true;
   return false;
}
...
void SonarScan::addSonarBeam(const SonarBeam& sonar_beam, bool resize) {
   if (memory_layout_column)
      throw std::runtime_error(
          "addSonarBeam: cannot add sonar beam because the memory layout is "
          "not supported. Call toggleMemoryLayout()");

   if (number_of_bins < sonar_beam.beam.size())
      throw std::runtime_error(
          "addSonarBeam: cannot add sonar beam: too many bins");
   ...
   memcpy(&data[index * number_of_bins], &sonar_beam.beam[0],
          sonar_beam.beam.size());
}
   ...
   if (memory_layout_column) {
      for (int row = 0; row < number_of_beams; ++row)
         for (int column = 0; column < number_of_bins; ++column)
            temp[row * number_of_bins + column] =
                data[column * number_of_beams + row];
   } else {
      for (int row = 0; row < number_of_beams; ++row)
         for (int column = 0; column < number_of_bins; ++column)
            temp[column * number_of_beams + row] =
                data[row * number_of_bins + column];
   }
...
uint32_t SonarScan::getNumberOfBytes() const { return data.size(); }

uint32_t SonarScan::getBinCount() const {
   return number_of_beams * number_of_bins;
}

const std::vector<uint8_t>& SonarScan::getData() const { return this->data; }

Angle SonarScan::getEndBearing() const {
   return start_bearing - angular_resolution * (number_of_beams - 1);
}

Angle SonarScan::getStartBearing() const { return start_bearing; }

Angle SonarScan::getAngularResolution() const { return angular_resolution; }
...
void SonarScan::setData(const std::vector<uint8_t>& data) { this->data = data; }

void SonarScan::setData(const char* data, uint32_t size) {
   if (size != this->data.size()) {
      std::cerr << "SonarScan: " << __FUNCTION__ << " (" << __FILE__
                << ", line " << __LINE__
                << "): " << "size mismatch in setData() (" << size
