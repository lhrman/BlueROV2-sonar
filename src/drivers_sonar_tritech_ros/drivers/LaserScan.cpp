#include "drivers_sonar_tritech/LaserScan.hpp"
#include <stdexcept>
...
bool LaserScan::isValidBeam(const unsigned int i) const {
   if (i > ranges.size()) throw std::out_of_range("Invalid beam index given");
...
bool LaserScan::isRangeValid(LaserScan::uint32_t range) const {
   if (range >= minRange && range <= maxRange &&
       range >= END_LASER_RANGE_ERRORS)
      return true;
   return false;
}

bool LaserScan::getPointFromScanBeamXForward(const unsigned int i,
   ...
   point =
       Eigen::Quaterniond(Eigen::AngleAxisd(
           start_angle + i * angular_resolution, Eigen::Vector3d::UnitZ())) *
       point;

   return true;
}

bool LaserScan::getPointFromScanBeam(const unsigned int i,
                                     Eigen::Vector3d& point) const {
   if (!isValidBeam(i)) return false;
...
std::vector<Eigen::Vector3d> LaserScan::convertScanToPointCloud(
