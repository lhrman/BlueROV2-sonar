#include "drivers_sonar_tritech/SeaNetEchosounder.hpp"
...
Echosounder::Echosounder() : iodrivers_base::Driver(50, true) {}
...
bool Echosounder::getDistance(base::samples::RigidBodyState& state,
                              int timeout) {
   iodrivers_base::Timeout time_out(timeout);
   while (!time_out.elapsed()) {
      size_t size =
          iodrivers_base::Driver::readPacket(buffer, 50, time_out.timeLeft());
      if (size > 0) {
         int depth = 0;
...
int Echosounder::extractPacket(uint8_t const* buff, size_t buffer_size) const {
   std::cout << "DEBUG: trying to extract package buffer size " << buffer_size;
   for (unsigned int i = 0; i < buffer_size; i++) {
      if (buff[i] == '\n') {
         std::cout << "DEBUG: Found packet of size " << i << std::endl;
         return i + 1;
      }
   }
   return 0;
