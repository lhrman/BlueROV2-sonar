#include "drivers_sonar_tritech/RigidBodyState.hpp"
...
Vector3d angularVelocity2EulerRate(const Vector3d& angular_velocity,
   ...
   // This is the transformation matrix that maps the angular velocity into an
   // Euler angle rate vector following the ZYX-order (yaw-pitch-roll).
   Eigen::Matrix3d ypr_jacobian;
   ypr_jacobian << 0, sr / cp, cr / cp, 0, cr, -sr, 1, sr * sp / cp,
       cr * sp / cp;
...
Vector3d eulerRate2AngularVelocity(const Vector3d& euler_rate,
   ...
   ypr_jacobian_inv << -sp, 0, 1, cp * sr, cr, 0, cp * cr, -sr, 0;
...
Pose RigidBodyState::getPose() const { return Pose(position, orientation); }

double RigidBodyState::getYaw() const { return base::getYaw(orientation); }

double RigidBodyState::getPitch() const { return base::getPitch(orientation); }

double RigidBodyState::getRoll() const { return base::getRoll(orientation); }

Vector3d RigidBodyState::getEulerRate() const {
   return base::angularVelocity2EulerRate(angular_velocity, orientation);
}

double RigidBodyState::getYawRate() const { return getEulerRate()[0]; }

double RigidBodyState::getPitchRate() const { return getEulerRate()[1]; }

double RigidBodyState::getRollRate() const { return getEulerRate()[2]; }
...
void RigidBodyState::initSane() { invalidate(); }
...
bool RigidBodyState::isValidValue(const Orientation& ori) {
   return !isNaN(ori.w()) && !isNaN(ori.x()) && !isNaN(ori.y()) &&
          !isNaN(ori.z()) &&
          fabs(ori.squaredNorm() - 1.0) <
              1e-6;  // assuming at least single precision
}

bool RigidBodyState::isKnownValue(const Matrix3d& cov) {
   return !isInfinity(cov(0, 0)) && !isInfinity(cov(1, 1)) &&
          !isInfinity(cov(2, 2));
}
...
bool RigidBodyState::hasValidPosition() const { return isValidValue(position); }
...
void RigidBodyState::invalidatePosition() { position = invalidValue(); }
...
bool RigidBodyState::hasValidVelocity() const { return isValidValue(velocity); }
...
void RigidBodyState::invalidateVelocity() { velocity = invalidValue(); }
...
void RigidBodyState::invalidateCovariances(bool invPos, bool invOri,
                                           bool invVel, bool invAngVel) {
   if (invPos) {
      invalidatePositionCovariance();
   }
   if (invOri) {
