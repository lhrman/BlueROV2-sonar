#include "drivers_sonar_tritech/Angle.hpp"
#include <boost/format.hpp>
#include "drivers_sonar_tritech/Eigen.hpp"
...
AngleSegment::AngleSegment(const Angle& start, double _width)
    : width(_width), startRad(start.getRad()), endRad(startRad + width) {
   if (width < 0)
...
bool AngleSegment::isInside(const Angle& angle) const {
   ...
   if (angleRad < startRad) angleRad += 2 * M_PI;
...
bool AngleSegment::isInside(const AngleSegment& segment) const {
   ...
   if (otherStart < startRad) otherStart += 2 * M_PI;

   double otherEnd = otherStart + segment.width;

   if (otherEnd <= endRad) return true;

   return false;
}

std::vector<AngleSegment> AngleSegment::getIntersections(
    const AngleSegment& b) const {
...
Angle AngleSegment::getStart() const { return Angle::fromRad(startRad); }

Angle AngleSegment::getEnd() const { return Angle::fromRad(endRad); }

std::ostream& operator<<(std::ostream& os, AngleSegment seg) {
   os << " Segmend start " << seg.startRad / M_PI * 180.0 << " end  "
      << seg.endRad / M_PI * 180.0 << " width " << seg.width / M_PI * 180.0;
   return os;
}
