#include "drivers_sonar_tritech/SeaNetProfiling.hpp"
#include "drivers_sonar_tritech/TimestampEstimator.hpp"
...
Profiling::Profiling() : SeaNet(PROFILINGSONAR) {
   timestamp_estimator = new aggregator::TimestampEstimator(
       base::Time::fromSeconds(2), base::Time::fromSeconds(0), INT_MAX);
}

Profiling::~Profiling() { delete timestamp_estimator; }
...
struct ProfilingHeadCommand {
   ...
   uint8_t filt_gain;
   uint8_t adaptive_gain_control_max;
   uint8_t adaptive_gain_control_set_point;
   uint16_t slope_ch1;
   uint16_t slope_ch2;
   uint8_t motor_step_delay_time;
   uint8_t motor_step_angle_size;
   uint16_t scan_time;
   uint16_t prf_spl;
   uint16_t prf_ctl2;
   uint16_t lockout_time;
   uint16_t minor_axis_dir;
   uint8_t major_axis_pan;
   uint8_t crtl2;
   uint16_t scan_z;
   uint8_t ad_threshold_ch1;
   uint8_t ad_threshold_ch2;
   uint8_t filt_gain_ch1;
   uint8_t filt_gain_ch2;
   uint8_t agc_max_ch1;
   uint8_t agc_max_ch2;
   uint8_t agc_set_point_ch1;
   uint8_t agc_set_point_ch2;
   uint16_t advanced_slope_ch1;
   uint16_t advanced_slope_ch2;
   uint16_t advanced_slope_delay_ch1;
...
void Profiling::configure(const ProfilingConfig& config, uint32_t timeout) {
   ...
   if (config.angular_resolution.rad < 0 ||
       ...
       config.frequency_chan1 < 600000 || config.frequency_chan1 > 1200000 ||
       config.frequency_chan2 < 600000 || config.frequency_chan2 > 1200000 ||
       config.select_channel < 1 || config.select_channel > 2 ||
       config.max_distance > 16383.0 || config.max_distance < 0.0) {
   ...
   if (config.left_limit.rad < 0.0 && config.right_limit.rad >= 0.0)
      opening_angle =
          config.left_limit.rad - (config.right_limit.rad - 2 * M_PI);
   else
      opening_angle = config.left_limit.rad - config.right_limit.rad;
   if (std::ceil(opening_angle / config.angular_resolution.rad) >
       (SEA_NET_MAX_PACKET_SIZE - 51) / 2.0) {
      throw std::runtime_error(
          "Profiling::Profiling: invalid configuration. To many samples per "
          "message. Decrease the opening angle or increase the angular "
          "resolution.");
   }
   ...
   if (!config.continous) head_control |= PRF_ALT;
   if (config.select_channel == 2) head_control |= CHAN2;
   if (config.mode == PROFILING_FIRST) head_control |= PRF_FIRST;
   ...
   profiling_head_config.rxn_ch1 =
       ((config.frequency_chan1 + 455000) * 4294967296) / 32e6;
   profiling_head_config.rxn_ch2 =
       ((config.frequency_chan2 + 455000) * 4294967296) / 32e6;
   uint16_t left_limit =
   ...
   HeadCommand* head_config =
       reinterpret_cast<HeadCommand*>(&profiling_head_config);
...
void Profiling::configureAcquisition(ProfilingAcquisitionConfig const& config,
                                     uint32_t timeout) {
   if (has_pending_data)
      throw std::runtime_error(
   ...
   std::vector<uint8_t> packet = SeaNetPacket::createPaket(
       device_type, mtHeadCommand, (uint8_t*)&msg, sizeof(msg));
...
void Profiling::decodeScan(base::samples::LaserScan& scan) {
   std::cout << "DEBUG: decoding profiling scan" << std::endl;
   ...
   || (profiling_head_config.head_control &(PRF_ALT | CHAN2 |  PRF_FIRST |
   HASMOT | PRF_MASTER)) !=(data.head_control &(PRF_ALT | CHAN2 |  PRF_FIRST |
   HASMOT | PRF_MASTER)))
   {
       std::cerr << "Configuration of the beam differs from the desired one."
   ; std::cerr << "left_limit " << profiling_head_config.left_limit << " / "
   << data.left_limit ; std::cerr << "rigth_limit " <<
   profiling_head_config.right_limit << " / " << data.right_limit ; std::cerr
   << "angle size " << profiling_head_config.motor_step_angle_size << " / " <<
   data.motor_step_angle_size ; std::cerr << "head_control " <<
   profiling_head_config.head_control << " / " << data.head_control ; throw
   std::runtime_error("Configuration of the beam differs from the desired
   one.");
   }*/
   ...
   scan.time =
       timestamp_estimator->update(base::Time::now());  // base::Time::now();
   scan.start_angle =
       base::Angle::fromRad(-((double)data.right_limit / 6399.0 * 2.0 * M_PI) +
                            M_PI)
           .getRad();
   scan.angular_resolution =
       ((double)data.motor_step_angle_size) / 6399.0 * 2.0 * M_PI;
   ...
   scan.speed = (data.data_count * scan.angular_resolution) /
                (scan.time - bak).toSeconds();  // data.scan_time.toSeconds();
   if (data.head_control & 0x04) {              // Do we scan left or right?
      for (unsigned int i = 0; i < data.data_count; i++) {
         scan.speed *= -1.0;
         double distance =
             (data.scan_data[(data.data_count * 2 - 1) - ((i * 2) + 1)] |
              (data.scan_data[(data.data_count * 2 - 1) - (i * 2)] << 8)) *
             1e-6 * speed_of_sound /
             2.0;  // Time in microsecounds to secounds (1e-6) * time of water
                   // in speed / twice the way (2.0)
         scan.ranges.push_back(distance * 1000.0);  // To millimeters
      }
   } else {
      for (unsigned int i = 0; i < data.data_count; i++) {
         double distance =
             (data.scan_data[(i * 2)] | data.scan_data[(i * 2) + 1] << 8) *
             1e-6 * speed_of_sound /
             2.0;  // Time in microsecounds to secounds (1e-6) * time of water
                   // in speed / twice the way (2.0)
         scan.ranges.push_back(distance * 1000.0);  // To millimeters
