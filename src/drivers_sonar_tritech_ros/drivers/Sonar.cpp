#include "drivers_sonar_tritech/Sonar.hpp"
#include <stdexcept>
...
Sonar Sonar::fromSingleBeam(Time time, Time bin_duration, Angle beam_width,
                            Angle beam_height, const std::vector<float>& bins,
                            Angle bearing, float speed_of_sound) {
...
void Sonar::pushBeam(const std::vector<float>& bins) {
   if (!timestamps.empty())
      throw std::invalid_argument(
          "cannot call pushBeam(bins): the structure uses per-beam timestamps, "
          "use pushBeams(time, bins) instead");
...
void Sonar::pushBeam(const Time& beam_time,
                     const std::vector<float>& beam_bins) {
...
void Sonar::pushBeamBins(const std::vector<float>& beam_bins) {
   if (beam_bins.size() != bin_count)
      throw std::invalid_argument(
          "pushBeam: the provided beam does not match the expected bin_count");
...
void Sonar::setBeam(unsigned int beam, const std::vector<float>& bins) {
   if (!timestamps.empty())
      throw std::invalid_argument(
          "cannot call setBeam(bins): the structure uses per-beam timestamps, "
          "use setBeams(time, bins) instead");
...
void Sonar::setBeam(unsigned int beam, const Time& beam_time,
                    const std::vector<float>& beam_bins) {
...
void Sonar::setBeam(unsigned int beam, const Time& beam_time,
                    const std::vector<float>& beam_bins, Angle bearing) {
...
void Sonar::setBeamBins(int beam, const std::vector<float>& beam_bins) {
   if (beam_bins.size() != bin_count)
      throw std::invalid_argument(
          "pushBeam: the provided beam does not match the expected bin_count");
   std::copy(beam_bins.begin(), beam_bins.end(),
             bins.begin() + beam * bin_count);
}

Angle Sonar::getBeamBearing(unsigned int beam) const { return bearings[beam]; }
...
void Sonar::getBeamBins(unsigned int beam,
                        std::vector<float>& beam_bins) const {
...
Sonar Sonar::getBeam(unsigned int beam) const {
   return fromSingleBeam(getBeamAcquisitionStartTime(beam), bin_duration,
                         beam_width, beam_height, getBeamBins(beam),
...
void Sonar::validate() {
   if (bin_count * beam_count != bins.size())
      throw std::logic_error(
          "the number of elements in 'bins' does not match the bin and beam "
          "counts");
   if (!timestamps.empty() && timestamps.size() != beam_count)
      throw std::logic_error(
          "the number of elements in 'timestamps' does not match the beam "
          "count");
   if (bearings.size() != beam_count)
      throw std::logic_error(
          "the number of elements in 'bearings' does not match the beam count");
}
...
Sonar::Sonar(SonarScan const& old, float gain)
    : time(old.time),
      ...
      bin_duration(
          Time::fromSeconds(old.getSpatialResolution() / old.speed_of_sound)),
   ...
   if (!old.polar_coordinates)
      throw std::invalid_argument(
          "there's not such thing as a non-polar sonar device, fix your "
          "driver");

   bins.resize(bin_count * beam_count);

   SonarScan scan(old);
   if (old.memory_layout_column) scan.toggleMemoryLayout();
...
SonarBeam Sonar::toSonarBeam(float gain) {
   ...
   std::vector<float>::iterator max =
       std::max_element(raw_data.begin(), raw_data.end());
   if (*max > 1)
      std::transform(raw_data.begin(), raw_data.end(), raw_data.begin(),
                     std::bind2nd(std::divides<float>(), *max));

   std::transform(raw_data.begin(), raw_data.end(), raw_data.begin(),
                  std::bind2nd(std::multiplies<float>(), 255 * gain));
