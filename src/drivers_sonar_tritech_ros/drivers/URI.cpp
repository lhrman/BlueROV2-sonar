...
URI URI::parse(string const& uri) {
   ...
   while (one_before_options != string::npos) {
      ...
      if (separator == string::npos || separator > options_end) {
         throw std::invalid_argument("invalid options syntax in " + uri +
...
URI::URI(string const& scheme, string const& host, int port, Options options)
    : m_scheme(scheme), m_host(host), m_port(port), m_options(options) {}

string URI::getScheme() const { return m_scheme; }
string URI::getHost() const { return m_host; }
int URI::getPort() const { return m_port; }
URI::Options const& URI::getOptions() const { return m_options; }
