#include "drivers_sonar_tritech/Pose.hpp"
...
Orientation removeYaw(const Orientation& orientation) {
   return Eigen::AngleAxisd(-getYaw(orientation), Eigen::Vector3d::UnitZ()) *
          orientation;
}
...
Orientation removePitch(const Orientation& orientation) {
   return Eigen::AngleAxisd(-getPitch(orientation), Eigen::Vector3d::UnitY()) *
          orientation;
}
...
Orientation removeRoll(const Orientation& orientation) {
   return Eigen::AngleAxisd(-getRoll(orientation), Eigen::Vector3d::UnitX()) *
          orientation;
}
...
PoseUpdateThreshold::PoseUpdateThreshold(double _distance, double _angle)
...
bool PoseUpdateThreshold::test(const Eigen::Affine3d& pdelta) {
   return test(pdelta.translation().norm(),
               Eigen::AngleAxisd(pdelta.linear()).angle());
}

bool PoseUpdateThreshold::test(const Eigen::Affine3d& a2b,
...
std::ostream& operator<<(std::ostream& io, Pose const& pose) {
   io << "Position " << pose.position.transpose() << " Orientation (RPY)"
      << getRoll(pose.orientation) << " " << getPitch(pose.orientation) << " "
...
std::ostream& operator<<(std::ostream& io, Pose2D const& pose) {
   ...
   io << "Position " << pose.position.transpose() << " Orientation (Theta) "
      << pose.orientation;
   return io;
}
