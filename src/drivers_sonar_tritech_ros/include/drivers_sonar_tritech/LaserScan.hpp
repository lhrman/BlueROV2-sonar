...
struct LaserScan {
    ...
    * This value is not normalised and depends on various factors, like
    * distance, angle of incidence and reflectivity of object.
    */
   ...
   /** converts the laser scan into a point cloud according to the given
    * transformation matrix, the start_angle and the angular_resolution. If the
    * transformation matrix is set to identity the laser scan is converted into
    * the coordinate system of the sensor (x-axis = forward, y-axis = to the
    * left, z-axis = upwards) If a scan point is outside of valid range all its
    * coordinates are set to NaN. Unfortunately invalid scan points can not be
    * skipped because this would invalidate the remission association
    */
   template <typename T>
   void convertScanToPointCloud(
       std::vector<T> &points,
      ...
      if (!skip_invalid_points) {
         for (unsigned int i = 0; i < ranges.size(); i++) {
            ...
            if (getPointFromScanBeamXForward(i, point)) {
               ...
               points.push_back(Eigen::Vector3d(base::unknown<double>(),
                                                base::unknown<double>(),
                                                base::unknown<double>()));
            }
         }
    ...
    */
   bool getPointFromScanBeamXForward(const unsigned int i,
                                     Eigen::Vector3d &point) const;

   /** \deprecated -
    * returns the points in a wrong coordinate system
    */
   bool getPointFromScanBeam(const unsigned int i,
                             Eigen::Vector3d &point) const;

   /** \deprecated -
    * returns the points in a wrong coordinate system
    */
   std::vector<Eigen::Vector3d> convertScanToPointCloud(
