...
class UDPServerStream : public FDStream {
  public:
   ...
   UDPServerStream(int fd, bool auto_close, struct sockaddr* si_other,
                   size_t* s_len);
   virtual size_t read(uint8_t* buffer, size_t buffer_size);
   virtual size_t write(uint8_t const* buffer, size_t buffer_size);
  ...
  protected:
   ...
   virtual std::pair<ssize_t, int> recvfrom(uint8_t* buffer, size_t buffer_size,
                                            int flags, sockaddr* s_other,
                                            socklen_t* s_len);
   /** Internal implementation of recvfrom to allow for mocking */
   virtual std::pair<ssize_t, int> sendto(uint8_t const* buffer,
                                          size_t buffer_size);
