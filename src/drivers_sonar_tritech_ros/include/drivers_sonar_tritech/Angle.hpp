...
class Angle {
  ...
  public:
    ...
    */
   static inline double rad2Deg(double rad) { return rad / M_PI * 180.0; }
    ...
    */
   static inline double normalizeRad(double rad) { return Angle(rad).rad; }
    ...
    * @param rad - angle in radians.
    */
   static inline Angle fromRad(double rad) { return Angle(rad); }
    ...
    */
   double inline getRad() const { return rad; }

   /**
    * @return canonical value of the angle in degrees
    */
   double inline getDeg() const { return rad / M_PI * 180; }
   ...
   void operator=(const Angle &other) { rad = other.rad; }
    ...
    * */
   inline Angle flipped() const { return Angle(rad).flip(); }
   ...
   /** Computes the unsigned angle of the rotation that makes +a+ colinear with
    * +b+
    */
   static Angle vectorToVector(const base::Vector3d &a,
...
class AngleSegment {
  public:
   ...
   bool split(const Angle &angle, AngleSegment &rest) { return false; }
    ...
    * @return A vector, containing a segment for each intersecting part of the
    * segments.
    * */
