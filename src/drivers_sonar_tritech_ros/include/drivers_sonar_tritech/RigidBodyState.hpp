 ...
 *
 * It takes the provided orientation into account in order to map the provided
 * angular velocity (represented by axis-angle) into Euler angles rate vector
 * following the ZYX-order (yaw-pitch-roll).
 *
 * It considers that "orientation" represents the orientation of the targetFrame
 * expressed in the sourceFrame, the same way it is defined in RigidBodyState.
 * The input angular_velocity is, then, considered to be expressed in the
 * sourceFrame.
 *
 ...
 *
 * It takes the provided orientation into account in order to map the provided
 * Euler rate vector following the ZYX-order (yaw-pitch-roll) into an angular
 * velocity (represented by axis-angle).
 *
 * It considers that "orientation" represents the orientation of the targetFrame
 * expressed in the sourceFrame, the same way it is defined in RigidBodyState.
 * The resulting angular_velocity is, then, expressed in the sourceFrame.
 *
 * For instance, if sourceFrame is "body" and targetFrame is "world", the
 * resulting angular velocity will be expressed in the "body" frame.
 */
...
struct RigidBodyState {
    ...
    * represented by the given matrix is a known value (not Inf in the
    * diagonal).
    */
