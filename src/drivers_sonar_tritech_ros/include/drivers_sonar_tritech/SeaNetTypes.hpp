...
struct ProfilingConfig {
   ...
   bool operator==(const ProfilingConfig &other) const {
      return (other.mode == mode && other.left_limit == left_limit &&
...
struct ProfilingAcquisitionConfig {
   ...
   ProfilingAcquisitionConfig() : gain(0.4), ad_threshold(0.2) {}
};
...
struct MicronConfig {
   ...
   bool operator==(const MicronConfig &other) const {
      return (other.left_limit == left_limit &&
              ...
              other.speed_of_sound == speed_of_sound && other.gain == gain &&
              other.low_resolution == low_resolution &&
              other.continous == continous && other.invert == invert);
   };
   bool operator!=(const MicronConfig &other) const {
      return !(other == *this);
   };
