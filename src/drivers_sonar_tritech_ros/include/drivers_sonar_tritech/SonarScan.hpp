...
struct SonarScan {
  public:
   ...
   SonarScan(uint16_t number_of_beams, uint16_t number_of_bins,
   ...
   void init(uint16_t number_of_beams, uint16_t number_of_bins,
             Angle start_bearing, Angle angular_resolution,
             bool memory_layout_column = true, int val = -1);
   ...
   // throws an exception if the sonar scan cannot hold the sonar beam and
   // resize is set to false otherwise the sonar scan will be resized (the start
   // angle is not changed!)
   //
   // the memory layout must be one sonar beam per row otherwise the function
   // will throw a std::runtime_error
   void addSonarBeam(const base::samples::SonarBeam &sonar_beam,
                     bool resize = true);

   // returns a sonar beam for a given bearing
   // throws an exception if the sonar scans holds no information for the given
   // bearing the memory layout must be one beam per row
   //
   // an exception is thrown if the sonar beam holds no information about the
   // given bearing
   void getSonarBeam(const Angle bearing, SonarBeam &sonar_beam) const;

   // this toggles the memory layout between one sonar beam per row and one
   // sonar beam per column to add sonar beams the memory layout must be one
   // sonar beam per row
    ...
    * @return Returns the overall number of bins (number_of_beams *
    * number_of_bins)
    */
